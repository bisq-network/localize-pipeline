...
collect_changed_translation_files() {
    ...
    extension_regex="\\.($(translation_file_extension_regex))$"
    git status --porcelain -- "$rel_input_folder" \
      | awk -v extension_regex="$extension_regex" '{
            path = substr($0, 4)
            if (index(path, " -> ")) {
                old_path = substr(path, 1, index(path, " -> ") - 1)
                new_path = substr(path, index(path, " -> ") + 4)
                if (old_path ~ extension_regex) print old_path
                if (new_path ~ extension_regex) print new_path
                next
            }
            if (path ~ extension_regex) print path
        }' \
      | sort -u
}
...
prepare_translation_source() {
    ...
    if [[ "$translation_source" == "git" ]]; then
        log "Translation source is 'git'. Skipping Transifex pull; using localization files already in the repository." "INFO"
...
# the tx CLI) or "git" (use the localization files already in the repo). Foreign OSS
...
# Maximum files per PR before splitting. CodeRabbit refuses to review a PR with
# more than 100 files, and an unreviewed batch is the one thing batching exists
# to prevent, so stay strictly below that cap with a margin for headroom.
DEFAULT_MAX_FILES_PER_PR=90
...
# Stage files from BATCH_FILES array, commit, push, and create a PR.
# Uses the outer-scope variables: BATCH_FILES, REMOTE, DEFAULT_BRANCH,
# FORK_OWNER, FORK_REPO_NAME_SHORT, UPSTREAM_REPO_NAME, TARGET_BRANCH_FOR_PR.
# Arguments: $1 = branch name, $2 = commit message, $3 = PR title
stage_and_submit_batch() {
    ...
    local PR_BODY="This pull request was automatically generated by the translation script and contains the latest localization updates from the configured translation source and model provider.
...
if [ -n "$TRANSLATION_CHANGES" ]; then
    ...
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        ...
        # Resolve fork identity once (avoid re-parsing in subshells).
        ...
        if [ -n "${FORK_REPO_NAME:-}" ]; then
            ...
            if [[ "$FORK_OWNER" != "$fork_repo_owner" ]]; then
                log "Error: 'origin' owner ('$FORK_OWNER') does not match FORK_REPO_NAME owner ('$fork_repo_owner')." "ERROR"
                exit 1
            fi
        fi
        ...
        BASE_PR_TITLE="${BASE_PR_TITLE:-Automated Translation Update - $(date +'%Y-%m-%d %H:%M:%S')}"
