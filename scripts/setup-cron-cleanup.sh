...
PROJECT_NAME=$(basename "$PROJECT_PATH" | sed 's/[^a-zA-Z0-9_-]/-/g' | sed 's/--*/-/g' | sed 's/^-//' | sed 's/-$//')
...
cat > "$CRON_FILE" <<'EOF'
#!/bin/bash
# Weekly Docker cleanup for localize-pipeline
# Auto-generated by setup-cron-cleanup.sh

cd PROJECT_PATH_Q_PLACEHOLDER && ./docker-cleanup.sh
EOF
PROJECT_PATH_REPLACEMENT=${PROJECT_PATH_Q//\\/\\\\}
PROJECT_PATH_REPLACEMENT=${PROJECT_PATH_REPLACEMENT//&/\\&}
sed -i "s|PROJECT_PATH_Q_PLACEHOLDER|$PROJECT_PATH_REPLACEMENT|g" "$CRON_FILE"
