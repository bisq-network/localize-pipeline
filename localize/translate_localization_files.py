...
5.  **Terminology Consistency Across Related Keys**: Use the full translated file you were given to render each domain concept with one consistent term across every related key. When a concept named inline in a step-by-step instruction also has its own dedicated field label or prompt whose key shares the same dotted prefix (for example a step in a `...how.info` value that tells the user to paste a transaction ID into a field whose label is `...proposalTxId` or `...lockupTxId`, or an action such as locking a bond referenced both in an instruction and in its button label), the instruction text MUST use the same wording for that concept as the field or button key, so the user is not directed to a differently named control. Detect and fix these mismatches even when only some of the related keys are in your review scope: align the wording to the term already established elsewhere in the translated file, without changing meaning and without ever overriding the placeholder, glossary, or format rules above.
