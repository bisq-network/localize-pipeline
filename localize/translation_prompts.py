...
- **Match grammatical number**: Render the value with the same grammatical number as the English source. Many count messages come as singular/plural variants of the same string (for example, keys ending in `.single`/`.plural` or `.one`/`.other`, such as "Used {{0}} time" versus "Used {{0}} times"). Use the singular grammatical form when the source is singular and the plural form when the source is plural; never reuse the plural wording for the singular case or vice versa when the target language distinguishes them. Do not invent a distinction the target language does not express: identical singular and plural target forms are acceptable when that language genuinely uses the same form for both.
- **Keep compound nouns as single words**: In languages that build compound nouns by joining words together (such as German, Norwegian, Danish, Swedish, and Dutch), render a multi-word English noun phrase as the target language's single closed compound rather than as separate words, following that language's orthography (for example, Norwegian "Address note" becomes "Adressenotat", not the split "Adresse notat"). Do not force compounding in languages that do not use it.
